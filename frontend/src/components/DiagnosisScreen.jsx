...
export default function DiagnosisScreen({
  ...
  const fattoriRischio = graph.nodes
    .filter(n => n.type === 'stile_vita')
      ...
      <div style={mob.page}>
        ...
        <div style={mob.scrollArea}>
          ...
          <div style={mob.box}>
            ...
            <div style={mob.chipGrid}>
              {fattoriRischio.map((node) => (
                <button
                  key={node.id}
                  style={{
                    ...mob.chip,
                    background: stileVita.has(node.id) ? colors.accentLight : 'transparent',
                    borderColor: stileVita.has(node.id) ? colors.borderActive : colors.border,
                    color: stileVita.has(node.id) ? colors.accent : colors.textSecondary,
                  }}
                  onClick={() => handleCheckboxChange(node.id)}
                >
                  {stileVita.has(node.id) && <span style={{ marginRight: 4 }}>&#10003;</span>}
                  {node.id}
                </button>
              ))}
            </div>
            ...
            {fattoriRischio.map((node) => (
              ...
              >
                <div style={{
                  ...
                  border: `1.5px solid ${stileVita.has(node.id) ? colors.accent : colors.textMuted}`,
                  background: stileVita.has(node.id) ? colors.accent : 'transparent',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  transition: 'all 150ms ease', flexShrink: 0,
                }}>
                  {stileVita.has(node.id) && (
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                  )}
                </div>
                <span style={{ lineHeight: 1.3 }}>{node.id}</span>
              </label>
            ))}
          </div>
        </div>
